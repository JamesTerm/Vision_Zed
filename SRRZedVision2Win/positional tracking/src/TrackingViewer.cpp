...
void GLViewer::exit() {
    ended_ = true;
	while (!vecWorldObj.empty())
	{
		delete vecWorldObj.back();
		vecWorldObj.pop_back();
	}
    glutLeaveMainLoop();
}
...
void GLViewer::init(sl::MODEL camera_model) {
	...
	// diplay from csv file
	std::ifstream inputFile;
	std::string csvName = "Detection_data";
	inputFile.open(csvName + ".csv");

	// bounds of world points
	sl::float3 bound_min(1000000.0f, 1000000.0f, 1000000.0f);
	sl::float3 bound_max(-1000000.0f, -1000000.0f, -1000000.0f);

	if (inputFile.is_open())
	{
		std::string input;
		// first line is description, throw away.
		...
		{	// get a line of values
			...
			try {
				...
				if (tok != NULL)
					point.x = strtof(tok, NULL);

				tok = strtok(NULL, ";");
				if (tok != NULL)
					point.y = strtof(tok, NULL);

				tok = strtok(NULL, ";");
				if (tok != NULL)
					point.z = strtof(tok, NULL);

				vecFilePath.emplace_back();
				vecFilePath.back() = point;
				...
				vecFilePathRot.emplace_back();
				vecFilePathRot.back() = point;

				// eat the tracking confidence
				tok = strtok(NULL, ";");
				...
				vecLocalPt.emplace_back();
				vecLocalPt.back() = point;
				...
				// get bounds TODO: look for clusters
				if (point.x < bound_min.x) bound_min.x = point.x;
				if (point.y < bound_min.y) bound_min.y = point.y;
				if (point.z < bound_min.z) bound_min.z = point.z;

				if (point.x > bound_max.x) bound_max.x = point.x;
				if (point.y > bound_max.y) bound_max.y = point.y;
				if (point.z > bound_max.z) bound_max.z = point.z;

				vecWorldPt.emplace_back();
				vecWorldPt.back() = point;
		...
		sl::float3 clr2(0.9f, 0.2f, 0.0f);
		for (int i = 0; i < vecLocalPt.size(); i++) {
			detectedTargets.addPoint(vecLocalPt[i], clr2);
		}
		sl::float3 clr3(0.9f, 0.9f, 0.0f);
		for (int i = 0; i < vecWorldPt.size(); i++) {
			detectedTargets.addPoint(vecWorldPt[i], clr3);
		}
		detectedTargets.setDrawingType(GL_POINTS);
		detectedTargets.pushToGPU();

		// show bounding box.
		targetBounds.clear();
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_min.z), clr3); // top
		targetBounds.addPoint(sl::float3(bound_max.x, bound_max.y, bound_min.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_min.z), clr3); // bottom
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_min.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_min.z), clr3); // left
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_min.z), clr3);
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_min.z), clr3); // right
		targetBounds.addPoint(sl::float3(bound_max.x, bound_max.y, bound_min.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_max.z), clr3); // top
		targetBounds.addPoint(sl::float3(bound_max.x, bound_max.y, bound_max.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_max.z), clr3); // bottom
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_max.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_max.z), clr3); // left
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_max.z), clr3);
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_max.z), clr3); // right
		targetBounds.addPoint(sl::float3(bound_max.x, bound_max.y, bound_max.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_min.z), clr3); // front to back
		targetBounds.addPoint(sl::float3(bound_min.x, bound_min.y, bound_max.z), clr3); 
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_min.z), clr3);
		targetBounds.addPoint(sl::float3(bound_min.x, bound_max.y, bound_max.z), clr3);
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_min.z), clr3); 
		targetBounds.addPoint(sl::float3(bound_max.x, bound_min.y, bound_max.z), clr3); 
		...
		targetBounds.setDrawingType(GL_LINES);
		targetBounds.pushToGPU();
	}
...
bool GLViewer::GetPoints() {
	bool ret = points_request;
	points_request = false;
	return ret;
}
...
void GLViewer::update() {
	...
	if (mouseButton_[MOUSE_BUTTON::MIDDLE]) {
		if (!points_request && mid_btn_up)
			points_request = true;
		mid_btn_up = false;
	}
	else
	...
	if (updateWorldData) {
		for (int i = 0; i < vecWorld.size(); i++) {
			vecWorldObj.back()->addPoint(vecWorld[i], vecColor[i]);
		}
		vecWorldObj.back()->pushToGPU();
		updateWorldData = false;
	}
    mtx.unlock();
}
...
void GLViewer::draw() {
	...
	detectedTargets.draw();

	targetBounds.draw();

	// Move the capture 3D model to correct position
	for (int w = 0; w < vecWorldObj.size(); w++)
	{ 
		glUniformMatrix4fv(shMVPMatrixLoc_, 1, GL_FALSE, (sl::Transform::transpose(vecWorldObj[w]->getModelMatrix()) *  sl::Transform::transpose(vpMatrix)).m);
		vecWorldObj[w]->draw();
	}
...
void GLViewer::updateWorld(sl::Transform zed_rt, sl::Mat point_cloud) {
	mtx.lock();
	sl::Resolution pres = point_cloud.getResolution();
	// TODO: need vec for color in image.
	vecWorld.clear();
	vecColor.clear();
	for (size_t x = 0; x < pres.width; x+=1)
	{
		for (size_t y = 0; y < pres.height; y+=1)
		{
			sl::float4 point3D;
			...
			{
				sl::float3 pnt(point3D.x, point3D.y, point3D.z);
				vecWorld.emplace_back();
				vecWorld.back() = pnt;

				union c color;
				color.f = point3D.w;
				sl::float3 clr(((float)color.s[0])/255, ((float)color.s[1])/255, ((float)color.s[2])/255);
				vecColor.emplace_back();
				vecColor.back() = clr;
			}
		}
	}
	Simple3DObject* p_obj = new Simple3DObject();
	if (p_obj != nullptr)
	{
		p_obj->setRT(zed_rt);
		p_obj->setDrawingType(GL_POINTS);
		vecWorldObj.push_back(p_obj);
...
void GLViewer::updateZEDPosition(sl::Transform zed_rt) {
 //   mtx.lock();
	//sl::float3 pnt = zed_rt.getTranslation();
	//sl::float3 last;
	//if (!vecPath.empty())
	//	last = vecPath.back();
	//if ((vecPath.empty()) ||
	//	pnt.distance(last, pnt) > 0.02f)
	//{
...
void Simple3DObject::pushToGPU() {
    if ((!isStatic_ || vaoID_ == 0) && vertices_.size() > 0) {
        if (vaoID_ == 0) {
