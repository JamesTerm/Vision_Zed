...
public:
	...
	bool GetPoints();
	void updateWorld(sl::Transform zed_rt, sl::Mat point_cloud);
...
private:
	...
	Simple3DObject detectedTargets;
	Simple3DObject targetBounds;

	std::vector<Simple3DObject*> vecWorldObj;

    std::vector<sl::float3> vecPath;
	std::vector<sl::float3> vecWorld;
	std::vector<sl::float3> vecColor;

	std::vector<sl::float3> vecFilePath;
	std::vector<sl::float3> vecFilePathRot;
	std::vector<sl::float3> vecLocalPt;
	std::vector<sl::float3> vecWorldPt;

    std::mutex mtx;
    bool updateZEDposition;
	bool updateWorldData;
